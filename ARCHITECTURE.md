...
## Architectural Decisions (ADRs)
 ...
 ### 2026-07-23 — Website Update + Security Incident (AUTH_TOKEN)
 - **Changed:** `website/src/app/page.tsx`, `website/src/app/globals.css`, `website/src/components/FaqSection.tsx`, `extension/background.js`, `backend/src/index.js`, `backend/tests/worker_logic.test.js`, `extension/manifest.json`, `extension/ui-spec.html`, `CONTEXT.md`, `CLAUDE.md`, `README.md`
 - **Why:** (1) Website updated with new 4-card "What's New" features section (Keyboard Shortcut, Per-Site Memory, Works Offline, Persistent Undo), updated FAQ (8 items), shortcut comparison bullet, footer tagline. (2) GitGuardian detected `AUTH_TOKEN` hardcoded in `background.js` in the public repo — security incident response.
 - **Patterns introduced:** `.features-4-grid` 4-column responsive card layout with scroll-reveal stagger; feature-card component pattern matching engine-col aesthetic; FAQ items 7-8 stagger rules.
 - **Security incident response:** Immediately removed `AUTH_TOKEN` from `background.js`, deleted the Cloudflare Worker secret via `wrangler secret delete AUTH_TOKEN`, removed the `Authorization` header check from `backend/src/index.js`, and deleted 3 now-invalid auth token tests. The worker now relies exclusively on CORS origin-checking (`isAllowedOrigin()`). The leaked token is fully invalidated.
 - **Mistakes caught:** Feature-card CSS had duplicate `transition` property across two rule blocks (layout block + stagger block) — removed from layout block. FAQ stagger only covered 6 items; extended to 8. Feature-card hover `transform` conflicted with stagger reset — added `!important`.
